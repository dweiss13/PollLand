...
<h1><center>Political Polls!</center></h1>
...
$sql = "SELECT * FROM question WHERE category = 'politics' AND id ORDER BY RAND()  
LIMIT 1";
    $result = $link->query($sql);
    
    if ($result->num_rows > 0){
      while($row = $result->fetch_assoc()) {
        echo "<center><form method='POST' action='politics.php'><h1>Polls Polls Polls!<h1><input hidden value='" . $row["id"] . "' name='quest'> <br>";
        $questid = $row["id"];
